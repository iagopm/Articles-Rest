...
class ArticleModifyTests {
		...
		void modifyArticle() throws Exception {
			String requestBody = "{\"id\":0,"
					//Se modifica este campo
					+ "\"name\":\"Zapatillas exquisitas ultimo modelo\","
					+ "\"description\":\"Tienen suela de goma\","
					+ "\"rating\":2,"
					+ "\"price\":99.99,"
					+ "\"brand\":\"PUMA\","
			...
			String expectedResponse = "Modified successfully";
			
			String response = mvc.perform(put("/api/modifyArticleById")
					.content(requestBody)
					.contentType(MediaType.APPLICATION_JSON))
			.andExpect(status().isOk()).andDo(print())
			.andReturn().getResponse().getContentAsString();
